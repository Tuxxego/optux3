...
extern ConVar    sk_plr_dmg_smg1_grenade;	 //  Variable for grenade launcher (Secondary) damage, both NPC and player (plr)
...
public:
	...
	float	GetFireRate( void ) { return 0.0625f; }	// Firerate, Lower means faster, higher means slower.
...
CWeaponSMG1::CWeaponSMG1( )
{
	...
	m_fMaxRange1		= 520; // Maxinum range.
...
{
	UTIL_PrecacheOther("grenade_ar2"); // Smg1 Grenade!
...
void CWeaponSMG1::Equip( CBaseCombatCharacter *pOwner )
{
	...
	{
		m_fMaxRange1 = 520; // If player
	}
	else
	{
		m_fMaxRange1 = 520; // else, npc
	}
...
void CWeaponSMG1::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	...
	case EVENT_WEAPON_SMG1:
		...
		break;
	// Purpose: Disabled due to bugs (default hl2)
...
//-----------------------------------------------------------------------------
// Purpose: Recoil, easy dampen means how dampened it is, max vertical kick is how high it kicks, slide limit is the time it lasts.
//-----------------------------------------------------------------------------
...
#define	SLIDE_LIMIT			2.0f	//Seconds - was 2.0
    // get fucked breadman (joke if you cant tell)
	//Get the view kick
...
//-----------------------------------------------------------------------------
// Purpose:  Ah, the secondary smg1 attack, the grenade launcher. Shes a real beast...
//-----------------------------------------------------------------------------
void CWeaponSMG1::SecondaryAttack( void )
{
	...
	BaseClass::WeaponSound( WPN_DOUBLE ); // Play Sound

	pPlayer->RumbleEffect( RUMBLE_357, 0, RUMBLE_FLAGS_NONE ); // Play Grenade Launcher Rumble (uses 357s)
	...
	// Decrease ammo
	pPlayer->RemoveAmmo( 1, m_iSecondaryAmmoType ); // Decrease "secondary" ammo type.
...
//================================
// not used yet, will update when it is implemented
//================================
// void CWeaponSMG1::BaseCustomization(void)
// {
...
//-----------------------------------------------------------------------------
// Npc proficiency, which I think can be configured in hammer, its been awhile since I used it!
//-----------------------------------------------------------------------------
